...
const CONTACT_WEBSITE = "www.makendi.coffee";
...
const text = {
  en: {
    ...
    reference: "Educational process information is rewritten from public industry references and is not a Makendi.coffee product specification. Final composition, quality, application fit, documentation and availability must be confirmed directly.",
    form: {
      eyebrow: "Product inquiry",
      title: "Tell us what you are developing.",
      copy: "Choose the format that interests you and share the market, application and approximate volume. Makendi.coffee will use the brief to continue the conversation.",
      name: "Name", company: "Company", email: "Work email", market: "Country / market", product: "Format of interest", application: "Application", volume: "Indicative volume", message: "Your brief", consent: "I agree that Makendi.coffee may use these details to respond to my inquiry.", submit: "Send inquiry", sending: "Sending", success: "Thank you. Your inquiry has been recorded. You can also send the prepared email for immediate direct follow-up.", error: "The inquiry could not be saved. Please use the prepared email to contact us directly.",
    },
  },
  tr: {
    ...
    reference: "Eğitici süreç bilgileri kamuya açık sektör kaynaklarından özgün biçimde yeniden yazılmıştır ve Makendi.coffee ürün spesifikasyonu değildir. Nihai bileşim, kalite, kullanım uygunluğu, belgeler ve bulunabilirlik doğrudan teyit edilmelidir.",
    form: {
      eyebrow: "Ürün talebi",
      title: "Ne geliştirdiğinizi bize anlatın.",
      copy: "İlgilendiğiniz formatı seçin; pazar, kullanım alanı ve yaklaşık hacmi paylaşın. Makendi.coffee bu özet üzerinden görüşmeyi sürdürecektir.",
      name: "Ad soyad", company: "Şirket", email: "İş e-postası", market: "Ülke / pazar", product: "İlgilenilen format", application: "Kullanım alanı", volume: "Tahmini hacim", message: "Talep özeti", consent: "Makendi.coffee'nin talebime yanıt vermek için bu bilgileri kullanmasını kabul ediyorum.", submit: "Talebi gönder", sending: "Gönderiliyor", success: "Teşekkürler. Talebiniz kaydedildi. Doğrudan takip için hazırlanan e-postayı da gönderebilirsiniz.", error: "Talep kaydedilemedi. Lütfen hazırlanan e-postayı kullanarak doğrudan iletişime geçin.",
    },
  },
};
...
function inquiryEmailHref(data, language) {
  const subject = language === "tr" ? "Makendi.coffee ürün talebi" : "Makendi.coffee product inquiry";
  const labels = language === "tr"
    ? { name: "Ad soyad", company: "Şirket", email: "E-posta", market: "Ülke / pazar", product: "Kahve formatı", application: "Kullanım alanı", volume: "Tahmini hacim", message: "Talep özeti" }
    : { name: "Name", company: "Company", email: "Email", market: "Country / market", product: "Coffee format", application: "Application", volume: "Indicative volume", message: "Brief" };
  const body = ["name", "company", "email", "market", "product", "application", "volume", "message"]
...
function HomePage({ language, copy }) {
  useMeta(language === "tr" ? "Makendi.coffee — Üç çözünebilir kahve formatı" : "Makendi.coffee — Three forms of instant coffee", language === "tr" ? "Sprey kurutulmuş, aglomere ve dondurularak kurutulmuş çözünebilir kahveyi keşfedin." : "Explore spray dried, agglomerated and freeze dried soluble coffee formats.");
...
function InquiryForm({ language, copy }) {
  ...
  const submit = async (event) => {
    ...
    setState({ status: "sending", message: "", emailHref: "" });
    try {
...
function Footer({language,copy}) { return <footer className="site-footer"><div className="shell footer-lead"><div><p className="eyebrow eyebrow--gold">Makendi.coffee</p><h2>{language === "tr" ? "Üç format. Net bilgi. Doğrudan talep." : "Three formats. Clear information. Direct inquiry."}</h2></div><Link className="button button--gold" to="/contact">{copy.inquiry}<ArrowRight aria-hidden="true" /></Link></div><div className="shell footer-grid"><div className="footer-brand"><BrandLockup footer /><p>{language === "tr" ? "Sprey kurutulmuş, aglomere ve dondurularak kurutulmuş çözünebilir kahve için bilgi platformu." : "An information platform for spray dried, agglomerated and freeze dried soluble coffee."}</p></div><div><strong>{language === "tr" ? "Keşfet" : "Explore"}</strong><Link to="/products">{copy.nav.products}</Link><Link to="/process">{copy.nav.process}</Link><Link to="/applications">{copy.nav.applications}</Link></div><div><strong>{language === "tr" ? "İletişim" : "Contact"}</strong><Link to="/contact">{copy.nav.contact}</Link><Link to="/privacy">{language === "tr" ? "Gizlilik" : "Privacy"}</Link><a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a><a href={`tel:${CONTACT_PHONE_HREF}`}>{CONTACT_PHONE}</a></div><div><strong>{language === "tr" ? "Web sitesi" : "Website"}</strong><span>{CONTACT_WEBSITE}</span><span>{language === "tr" ? "Özel alan adı daha sonra bağlanacak" : "Custom domain will be connected later"}</span><span>{language === "tr" ? "Şu anda Vercel üzerinden yayında" : "Currently live on Vercel"}</span></div></div><div className="shell footer-base"><span>© {new Date().getFullYear()} Makendi.coffee</span><span>{language === "tr" ? "Geçici yapay zekâ görselleri ürün fotoğraflarıyla değiştirilecektir" : "Temporary AI imagery will be replaced with product photography"}</span></div></footer>; }
